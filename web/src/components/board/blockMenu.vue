    ...
    >
      ...
      <div class="item-handle ai-handle">
        ...
        <div class="ai-handle-menu">
          ...
          <div class="ai-handle-item translate">
            <div class="item-part">
              <div class="item-icon fa-solid fa-language"></div>
              <div>{{ "翻译为" }}</div>
            </div>
            <div class="ai-last-icon">
              <i class="fa-solid fa-arrow-turn-down"></i>
            </div>
            <div class="translate-select">
              <div class="translate-item" @click="aiBlockHandle('trans','trans2en')">英文</div>
              <div class="translate-item" @click="aiBlockHandle('trans','trans2chn')">中文</div>
            </div>
          </div>


          <div class="ai-handle-item" v-for="aiItem in aiItemData"  @click="aiBlockHandle(aiItem.type)">
            <div class="item-part">
...
export default {
  ...
  computed: {
    ...
    isShow() {
      console.log(
        "blockMenuData.visible",
        this.$store.state.page.note.blockMenuData.visible
      );
  ...
  methods: {
    ...
    //块级AI操作
    aiBlockHandle(type, suply=null) {
      let data = {
        type: type,
        id: this.now_block_id,
      };

      if (type="trans") {
        data.trans_type = suply
      }
      
      this.$store.dispatch("page/note/fetchBlockAi", data);
...
.translate:hover .translate-select{
  visibility: visible;
}
.translate-item{
  width: 100%;
