...
import * as ai_api from '@/views/page/ai_api.js'
...
export default {
  ...
  actions: {
    ...
    //块级ai操作
    async fetchBlockAi({ commit, state, dispatch }, data) {
      let res = await ai_api.HANDLE_AI(data);
      dispatch("fetchPageDetail", state.now_page_id);
    },

    //ai答复
    async fetchQaAi({ commit, state, dispatch }, data) {
      let res = await ai_api.QA_AI(data);
      dispatch("fetchPageDetail", state.now_page_id);
    },


    //知识库问答
    async fetchKnowledgeQa({ commit, state, dispatch }, data) {
      let res = await ai_api.KNOWLEDGE_QA(data);
      dispatch("fetchPageDetail", state.now_page_id);
    },
    ...
    //修改页面
    async fetchUpdatePage({ commit, state, dispatch }, data) {
      let res = await api.UPDATE_PAGE(data);
      dispatch("fetchPageList");
    },
